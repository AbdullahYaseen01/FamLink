...
export const ShareTypeLabel = ({ role, goal }) => (
  <>
    <span className="Livvic-Bold">{role}</span>
    <span className="opacity-40">·</span>
    <span className="Livvic">{goal}</span>
  </>
);
...
export const ShareTypeBadge = ({ variant, className = "" }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 Livvic rounded-full px-2.5 sm:px-3 py-1 text-[10px] sm:text-[11px] md:text-xs min-w-0 max-w-full ${className}`}
