...
export default function Component() {
  ...
  const { incomingMatches, isMatchLoading } = useSelector(
  ...
  return (
    <div>
      ...
      <div className="padding-navbar1 !py-8 sm:!py-12">
        <div className="flex justify-between items-center mb-4 px-2 sm:px-0">
          <div>
            <div className="flex gap-2 items-center">
              <h1 className="Livvic-SemiBold text-xl sm:text-2xl lg:text-3xl mb-1">Requests</h1>
              <div className="bg-purple-500 py-0.5 px-2 sm:py-1 sm:px-2 rounded-lg Livvic-Medium text-sm sm:text-base text-white">
                {incomingMatches?.filter((match) => match.status === "pending").length}
              </div>
            </div>
            <p className="Livvic-Medium text-gray-400 text-xs sm:text-sm lg:text-lg">
          ...
          <div className="flex gap-3 sm:gap-6 items-center">
            <div
              ...
              className="cursor-pointer Livvic-SemiBold text-sm sm:text-base"
            >
            ...
            >
              <div className="Livvic-SemiBold text-sm sm:text-base">View All</div>
            </NavLink>
          </div>
        </div>
        <div className="max-w-7xl mx-auto">
          {isCollapse && (
            <ChatInterfaceRequests
              matches={incomingMatches.filter((match) => match.status === "pending")}
      ...
      {chatList.filter((c) => c.otherParticipant?.type !== "Admin").length > 0 ? (
        <div className="padding-navbar1 border-t border-[#EEEEEE] flex bg-background h-[calc(100vh-80px)] overflow-hidden">
          {/* Contact List */}
          <div className={`w-full md:w-1/3 border-r ${selectedContact ? "hidden md:block" : "block"}`}>
            <div className="flex bg-white justify-between items-center px-3 sm:px-4 py-4 sm:py-6">
              <div>
                <div className="flex bg-white gap-2 items-center mb-1">
                  <h1 className="Livvic-SemiBold text-xl sm:text-2xl lg:text-3xl">Conversations</h1>
                  <div className="bg-purple-500 py-0.5 px-2 sm:py-1 sm:px-2 rounded-lg Livvic-Medium text-sm sm:text-base text-white">
                    {filteredContacts?.filter((contact) => contact?.otherParticipant?.name !== "Admin").length}
                  </div>
                </div>
                <p className="Livvic-Medium text-gray-400 text-xs sm:text-sm lg:text-base">
                  Accept Match request to chat.
                </p>
              </div>
            </div>
            <div className="px-3 sm:px-4 relative">
              <input
                type="text"
                value={searchQuery}
        ...
        <div className="padding-navbar1 !pb-6 h-[calc(100vh-500px)] mt-8 sm:mt-12">
          <div className="flex gap-2 items-center mb-1">
            <h1 className="Livvic-SemiBold text-xl sm:text-2xl lg:text-3xl">Conversations</h1>
            <div className="bg-purple-500 py-0.5 px-2 sm:py-1 sm:px-2 rounded-lg Livvic-Medium text-sm sm:text-base text-white">0</div>
          </div>
          <p className="Livvic-Medium text-gray-400 text-xs sm:text-sm lg:text-base">
            Accept Match request to chat
          </p>
          <div className="flex justify-center items-center w-full h-full text-sm sm:text-base text-gray-500">
