...
/*
 * The nanny "looking for a share position" wizard: five steps, Q1-Q18, one
 * container.
 *
 * Replaces LookingForJob/Screen4.jsx and the six CompleteProfile/Step*.jsx it
 * routed to. Those steps each owned an antd Form hoisted to the container
 * through jobFormRef, with a separate plain formValues object and a daysState
 * kept outside antd entirely — and Back just decremented currentStep without
 * re-seeding anything, so every answer on the step you returned to was gone.
 * All answers live here and the step components hold none, which is what makes
 * Back free.
 */

/*
 * Keyed by schema field name wherever possible, so the payload builder is close
 * to a passthrough. The two exceptions are named for what they are:
 * preferredAgeLabels holds the raw Q4 strings (the {label,min,max} conversion is
 * a payload concern) and photoFile/photoPreviewUrl never reach the document.
 */
...
export default function NannyShareOnboardingWizard() {
  ...
  /* No useNavigate: completion does not redirect, so the finished state renders
     in place and CompleteScreen carries the one CTA. */
  ...
  async function submit() {
    ...
    /* budget.sharedRate.{min,max} is the only nanny rate path the browse filter
       ...
       every narrowed search. Bounce back to the question rather than saving it. */
    if (!rateIsUsable(fields.budget)) {
      setErrors({ q12: "Please reselect your shared-care rate." });
      setStep(4);
      window.scrollTo({ top: 0, behavior: "smooth" });
      return;
    }
